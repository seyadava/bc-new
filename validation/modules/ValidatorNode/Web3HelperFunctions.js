...
function getAdmins() {
    validatorContract.getAdmins(function (error, result) {
        if (!error) {
            var adminsList = [];
            // Filter out the default value addresses
...
function addValidators(fromAccountAddress, addresses, privateKey) {
...
function addValidator(fromAccountAddress, newValidatorAddress, privateKey) {
    var methodData = validatorContract.addValidators.getData([newValidatorAddress]);
    sendMethodTransaction(fromAccountAddress, methodData, privateKey);
}
