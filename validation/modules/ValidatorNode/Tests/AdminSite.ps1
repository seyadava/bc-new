...
function Get-ParseAdminTableHtmlToObject() {
    ...
    foreach ($row in $rows) {
        ...
        $addressHeader = $cells[1].innerText -eq "Address"
        $PeerCountHeader = $cells[2].innerText -eq "Peer Count"
        $LatestBlockHeader = $cells[3].innerText -eq "Latest Block Number"

        if (!$nodeHostHeader -OR !$addressHeader -OR !$PeerCountHeader -OR !$LatestBlockHeader) {
            # Host Name
            $object.HostName = $cells[0].innerText
            # Peer
            $object.PeerCount = [convert]::ToInt32($cells[2].innerText, 10)
            # Last Block Number
            $object.LatestBlockNumber = [convert]::ToInt32($cells[3].innerText, 10)     
