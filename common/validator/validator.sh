...
# Appends node metadata of the current node to azure storage blob
publish_node_metadata() {
    ...
    if [[ $enodeUrl =~ ^enode ]]; then
        hostname=$(hostname);
        local nodeAddress=$1;
         # Look up the Azure region for this VM using Azure "Instance Metadata Service"
        local region=$(curl -s -H Metadata:true http://169.254.169.254/metadata/instance?api-version=2017-04-02 | jq -r .compute.location);
        echo "{\"passphraseUri\": \"${PASSPHRASE_URI}\", \"enodeUrl\": \"${enodeUrl}\", \"hostname\": \"$hostname\", \"nodeAddress\": \"$nodeAddress\", \"region\": \"$region\"}" > nodeid.json;
...
run_parity()
{
    ...
    # Convert BLOCK_RESEAL_MAX_PERIOD_IN_SEC to milliseconds
    block_reseal_max_period_ms=$(expr $BLOCK_RESEAL_MAX_PERIOD_IN_SEC \* 1000);
    ...
    cp $HOMEDIR/node.toml $CONFIGDIR/node.toml
    # Inject configurable Parity variables into node.toml
    sed -i "s/#ENGINE_SIGNER/$address/" $CONFIGDIR/node.toml
    sed -i "s/#ETH_RPC_PORT/$RPC_PORT/" $CONFIGDIR/node.toml
    sed -i "s/#ETH_WEBSOCKET_PORT/$WEBSOCKET_PORT/" $CONFIGDIR/node.toml
    sed -i "s/#BLOCK_RESEAL_MAX_PERIOD_IN_SEC/$block_reseal_max_period_ms/" $CONFIGDIR/node.toml
...
####################################################################################
if [ $# -lt 18 ]; then unsuccessful_exit "Insufficient parameters supplied." 56; fi
...
WEBSOCKET_PORT=$8;
PASSPHRASE=$9
PASSPHRASE_FILE_NAME=${10}
PASSPHRASE_URI=${11}
MODE=${12}
LEASE_ID=${13}
CONSORTIUM_DATA_URL=${14}
MUST_DEPLOY_GATEWAY=${15}
PARITY_LOG_FILE_PATH=${16}
DEBUG_MODE=${17}
BLOCK_RESEAL_MAX_PERIOD_IN_SEC=${18}
