...
Shared Key for Connection (Joining Existing & Private VNet)|Used to establish encryption for the connection. The same shared key must be used in both the virtual network gateways| Between 1 & 128 alphanumeric characters|NA
VNet Gateway (Joining Existing & Private VNet)|Resource Id of the Virtual Network Gateway to connect to on the leader deployment|Valid Azure Resource ID|NA
...
If you have deployed your network using the default "Deploy using Public IP" setting, and you are currently using the [Standard Tier of Security Center](https://docs.microsoft.com/en-us/azure/security-center/security-center-pricing), you can use the [JIT (Just in time)](https://docs.microsoft.com/en-us/azure/security-center/security-center-just-in-time) access feature to SSH into your VMs. 

Otherwise, to access the virtual machine instances in the PoA
network, you will need to change a network group
security rule to \"Allow\" using the steps below.
...
Ether doesn't serve a purpose in a Proof-of-Authority consortium. Hence there is no need to specify a gas fee when submitting transactions in MetaMask.
...
#### How do I backup a validator node
You can use Azure Recovery Services to maintain a backup of your validatod nodes.  [Follow this guide to enable backups](https://docs.microsoft.com/en-us/azure/backup/backup-azure-vms-first-look-arm)
