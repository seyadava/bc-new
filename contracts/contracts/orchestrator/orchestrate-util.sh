...
generate_poa_spec()
{
    ...
    # Convert block gas limit to hex
    eth_block_gas_limit='0x'$(printf "%x\n" $blockGasLimit  | awk '{print toupper($0)}');
    ...
    sed s/#BLOCK_GAS_LIMIT/$eth_block_gas_limit/ spec2.json > spec3.json;
    sed s/#TRANSACTION_PERMISSION_CONTRACT/$transactionPermissionContract/ spec3.json > $PARITY_SPEC_BLOB_NAME;
